...
class _AppointmentbookingState extends State<Appointmentbooking>
  ...
  Future<String?> _getChildIdByName(String childName) async {
    try {
      final userId = supabase.auth.currentUser?.id;
      ...
      final response = await supabase
          ...
          .maybeSingle();
      return response != null ? response['child_id']?.toString() : null;
    } catch (e) {
      debugPrint("Error fetching child id: $e");
      return null;
    }
  }
    ...
    try {
      ...
      setState(() {
        _availableDates = data
            .map((e) => e['availability_date'].toString())
            .toList();
      });
    } catch (e) {
  ...
  Future<void> _submitBooking() async {
    ...
    try {
      ...
      await supabase.from('tbl_appointment').insert({
        ...
        'child_id': await _getChildIdByName(_selectedChild!), // Store child_id for better data integrity
